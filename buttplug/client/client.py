...
class ButtplugClient(ButtplugClientConnectorObserver):
    ...
    async def _handle_message(self, msg: ButtplugMessage):
        ...
        # print('msg dict: ', self._msg_tasks)
        
        # print(f'msg counter: {self._msg_counter}')
        # msg.id = self._msg_counter - 1
        
        
#        print(f'in tasks? {msg.id in self._msg_tasks}')
        if msg.id == 0 and isinstance(msg, ServerInfo):
            self._msg_tasks[1].set_result(msg)
        elif msg.id in self._msg_tasks.keys() and not self._msg_tasks[msg.id].done():
            self._msg_tasks[msg.id].set_result(msg)
            # print(f'set result for {msg.id}')
            return
